...
def get_missing_steps(row, steps):
    ...
    ##############################################################################
    for i in steps:
        key = f'step{i}_missing'
        if key in row and int(row[key]) == 1:
...
def compute_v_S(df, all_subsets_missing):
    ...
    for S in all_subsets_missing:
        mask = df['missing_steps'].apply(lambda t: t == S)
        vals = df.loc[mask, 'is_correct']
        v_S[S] = float(vals.mean()) if len(vals) > 0 else np.nan
    return v_S
...
def compute_marginal_contributions(steps, v_S):
    ...
    for pi in permutations:
        ...
        for i in steps:
            ...
            #############################################################################################
            # S_i: set of steps that precede i in permutation pi (these are PRESENT before adding i)
            S_i = set(pi[:idx_i])
            # Represent in terms of "missing steps" (keys of v_S):
            missing_S_i = total_steps_set - S_i
            missing_S_i_union_i = missing_S_i - {i}
            ...
            if np.isnan(v_S_i) or np.isnan(v_S_i_union_i):
                ...
                ###############################################################################
                # Using missing-steps representation: contribution of adding i equals
                # v(M) - v(M \ {i}) where M is the current missing set before adding i.
                Delta_sum[i] += (v_S_i - v_S_i_union_i)
...
def compute_shapley_values(Delta_sum, valid_permutations_count, steps):
    ...
    if valid_permutations_count == 0:
        shapley_values = {i: np.nan for i in steps}
    else:
...
def main():
    ...
    for i in steps:
        print(f"Step {i} Shapley value: {shapley_values[i]:.6f}")
